...
# Caclulate the ensemble forecasts
def ensemble(models, weights, tickers=None):
    ...
    for name, forecasts in models.items():
        results[["Rank1", "Rank2", "Rank3", "Rank4","Rank5"]] += weights[name] * forecasts[["Rank1", "Rank2", "Rank3", "Rank4","Rank5"]]
...
class Evaluator:
    ...
    def __init__(self, dates_valid, dates_backtest, monthly_returns, returns_df, cleaned_closing_prices, closing_prices_df, train, test):
    ...
    def eval_models(self):
        ...
        self.roc_results = pd.DataFrame(
            index=[d for d in self.dates_backtest],
            columns=[config['name'] for config in self.configs]+['ensemble'],
            dtype=float
        )
        self.rps_results = pd.DataFrame(
            index=[d for d in self.dates_backtest],
            columns=[config['name'] for config in self.configs]+['ensemble'],
            dtype=float
        )
        self.all_forecasts = pd.DataFrame(
            index=pd.MultiIndex.from_tuples([(d, asset) for d in self.dates_backtest for asset in self.monthly_returns.columns], names=['Date', 'Asset']),
            columns=pd.MultiIndex.from_tuples([(m, r) for m in [config['name'] for config in self.configs]+['ensemble','truth'] for r in ["Rank1", "Rank2","Rank3", "Rank4", "Rank5"]], names=['Models', 'Ranks']),
            dtype=float
        )
        ...
        for d in tqdm(self.dates_backtest):
            ...
            y_true = pd.qcut((self.closing_prices_df[(self.closing_prices_df.index == date_range[-1])].values / self.closing_prices_df[(self.closing_prices_df.index == date_range[0])].values - 1).reshape(-1,), 5, duplicates='drop', labels=False)
            ...
            for config in self.configs:
                ...
                # Forecast next 4-weeks
                forecast = config['model'].forecast(test_df)
                self.forecasts[config['name']] = forecast

                # Save elapsed time for each model
                self.time[config['name']].loc[d] = time.time() - start

                roc_auc = roc_auc_score(to_categorical(y_true), forecast[["Rank1", "Rank2","Rank3", "Rank4", "Rank5"]].values, average='macro', multi_class='ovr')
                t = RPS_calculation(hist_data = d_hist_data, submission = self.forecasts[config['name']])


                # Save forecast and calculate Performance indicators
                self.rps_results[config['name']].loc[d] = t['RPS']
                self.roc_results[config['name']].loc[d] = roc_auc
            ...
            if bool(self.ensemble_weights) and len(self.forecasts.keys()) > 1:
                enseble_forecasts = ensemble(self.forecasts, self.ensemble_weights, tickers=eval_monthly_return_df.columns)
                t = RPS_calculation(hist_data = d_hist_data, submission = enseble_forecasts)
                roc_auc = roc_auc_score(to_categorical(y_true), enseble_forecasts[["Rank1", "Rank2","Rank3", "Rank4", "Rank5"]].values, average='macro', multi_class='ovr')

                self.rps_results['ensemble'].loc[d] = t['RPS']
                self.roc_results['ensemble'].loc[d] = roc_auc
                self.all_forecasts.loc[d, ('ensemble', )] = enseble_forecasts[["Rank1", "Rank2","Rank3", "Rank4", "Rank5"]].values


    def get_ensemble_weights(self, TOP_N_MODELS):

        ensemble_weights = pd.DataFrame(
            index=[d for d in self.dates_valid],
            columns=[config['name'] for config in self.configs],
            dtype=float
        )
        ...
        for d in tqdm(self.dates_valid):
            ...
            for config in self.configs:
                ...
                validation_forecasts[d][config['name']] = forecast
                validation_y[d] = d_hist_data

                t = RPS_calculation(hist_data = d_hist_data, submission = forecast)

                # Save forecast and calculate Performance indicators
                ensemble_weights[config['name']].loc[d] = t['RPS']

        for config in self.configs:
            self.ensemble_weights[config['name']] = np.mean(ensemble_weights[config['name']])

        self.ensemble_weights = {k: 1 / TOP_N_MODELS if w >= heapq.nlargest(TOP_N_MODELS, self.ensemble_weights.values())[-1] else 0 for k, w in self.ensemble_weights.items() }
    ...
    def save_results(self, results_file):
        ...
        with pd.ExcelWriter(results_file) as writer:
            self.rps_results.to_excel(writer, sheet_name='RPS')
            self.roc_results.to_excel(writer, sheet_name='ROC')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--YAHOO', nargs='?', type=int, const=0)
    parser.add_argument('--M6', nargs='?', type=int, const=1)
    parser.add_argument('--ENSEMBLE', nargs='?', type=int, const=0)
    parser.add_argument('--TUNING', nargs='?', type=int, const=0)
    parser.add_argument('--TOP_N_MODELS', nargs='?', type=int, const=4)
    ...
    eval = Evaluator(dates_valid, dates_backtest, monthly_returns, returns_df, cleaned_closing_prices, closing_prices_df, train, test)
    ...
    if len(models_configs) > 1 and args.ENSEMBLE:
        eval.get_ensemble_weights(args.TOP_N_MODELS)

    eval.eval_models()
    eval.save_results(results_file)
